...
interface ResourceFile {
  path: string;
  icon: string;
  label?: string; // Si absent, le nom sera extrait du fichier
}
 ...
 */
function createDownloadLink(file: ResourceFile): HTMLAnchorElement {
 ...
 */
function renderSections(config: ResourcesConfig, container: HTMLElement): void {
...
async function init(): Promise<void> {
  try {
    ...
    const config: ResourcesConfig = await response.json();
